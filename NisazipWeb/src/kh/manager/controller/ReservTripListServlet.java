...
@WebServlet("/reservTripList.mg")
public class ReservTripListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public ReservTripListServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(condition.equals("tId")) {
			listCount = mService.searchTIdCount(keyword);
		}else {
			listCount = mService.getTripReservListCount();
		}
		...
		if(condition.equals("tId")) {
			list = mService.searchTId(currentPage, limit, keyword);
		}else {
			list = mService.getTripReservList(currentPage, limit);
		}
		

		if(list!=null){
			page="views/manager/tripReserv_mng.jsp";
