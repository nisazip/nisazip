...
public class RInsertServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String R_TYPE = String.join(", ", request.getParameterValues("R_TYPE"));
		String R_TYPE2 = String.join(", ", request.getParameterValues("R_TYPE2"));
		int R_MAX_NUM = Integer.parseInt(request.getParameter("R_MAX_NUM"));
		
		HttpSession session = request.getSession();
		Room room = new Room();
		room.setR_type(R_TYPE);
		room.setR_type2(R_TYPE2);
		room.setR_max_num(R_MAX_NUM);
		
		System.out.println("1단계.");
		
		session.setAttribute("room", room);
		request.getRequestDispatcher("views/room/RoomInsert2.jsp").forward(request, response);		
