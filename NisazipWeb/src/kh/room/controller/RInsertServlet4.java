 ...
 */
@WebServlet("/rInsert4.do")
public class RInsertServlet4 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public RInsertServlet4() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (ServletFileUpload.isMultipartContent(request)) {
			...
			String pno = new rService().insertThumnail(room, fileList, m);
			
			if (pno != null) {
				session.setAttribute("pno", pno);

				request.getRequestDispatcher("/views/room/RoomInsert5.jsp").forward(request, response);
			} else {
				// 실패시 저장된 사진 삭제
			...
			/*// service로 전송
			int result = new rService().insertThumbnail(room, fileList);

			System.out.println(result);

			if (result > 0) {
				response.sendRedirect(request.getContextPath() + "/view/Roomlist.jsp");
			} else {
				// 실패시 저장된 사진 삭제
				for (int i = 0; i < saveFiles.size(); i++) {
					// 파일시스템에 저장된 이름으로 파일 객체 생성함
